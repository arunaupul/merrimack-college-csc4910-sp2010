...
GlWindow::~GlWindow()
{
	KillWindow();														// Call killwindow in case the user hasn't
}
...
bool GlWindow::CreateGlWindow( const std::wstring & className , const std::wstring & windowTitle , HINSTANCE hInstance , int windowPosX , int windowPosY , int windowWidth , int windowHeight , const bool & fullScreen , int bitsPerPixel , LPVOID application  )
{
	...
	if( m_isFullScreen )
	{
		EnableFullScreen( m_windowHeight , m_windowWidth , bitsPerPixel );
	}
	...
	windowRect.bottom = windowPosY + m_windowHeight;
	windowRect.right = windowPosX + m_windowWidth;
	...
	ResizeGlScene( m_windowWidth , m_windowHeight );
...
void GlWindow::ResizeGlScene( int width , int height )
{
	...
	if( m_windowHeight == 0 )									// Prevent 0 height windows
	{
		m_windowHeight = 1;
	}
